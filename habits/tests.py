...
class HabitTestCase(APITestCase):
    def setUp(self):
        ...
        self.habit = Habit.objects.create(habit='приятная привычка для теста',
                                          author=self.user,
                                          place='в приятном для теста месте',
                                          habit_time="07:07:07",
                                          lead_time_new="77", is_nice=True,
                                          period='1')
    ...
    def test_get_publish_list(self):
        self.data = {'habit': 'полезная привычка для теста',
                     'author': self.user.id,
                     'place': 'в полезном для теста месте',
                     'habit_time': '08:08:08',
                     'lead_time_new': '88', 'is_nice': 'False',
                     'period': '2', 'bonus': 'еще один тест',
                     'is_publish': 'True'}
    ...
    def test_post(self):
        ...
        self.assertTrue(Habit.objects.filter(
            habit='полезная привычка для теста').exists())

    def test_update(self):
        self.data = {'habit': 'измененная привычка для теста',
                     'author': self.user.id,
                     'place': 'в измененном для теста месте',
                     'habit_time': '09:09:09',
                     'lead_time_new': '99', 'is_nice': 'False',
                     'period': '2', 'bonus': 'еще один тест',
                     'is_publish': 'True'}
